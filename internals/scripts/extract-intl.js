...
const { appLocales, DEFAULT_LOCALE } = require("../../src/config/i18n");
...
for (const locale of appLocales) {
  ...
  const translationFileName = `src/config/i18n/translations/${locale}.json`;
  try {
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir("-p", "src/config/i18n/translations"); // eslint-disable-line

    let localeTaskDone;
    let translationFileName;

    for (const locale of appLocales) {
      translationFileName = `src/config/i18n/translations/${locale}.json`;
      localeTaskDone = task(
